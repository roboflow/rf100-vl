...
def evaluate_predictions(dataset_path, predictions_path):
    ...
    if len(predictions) == 0:
        print("Warning: No predictions to evaluate - assigning zero scores")
        # Return zero metrics for datasets with no predictions
        metrics = {
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate SAM3 predictions")
    parser.add_argument("--rf100_dir", type=str, default="rf100-vl/rf100-vl",
                        help="Path to RF100-VL datasets directory")
    parser.add_argument("--predictions_dir", type=str, default="predictions/sam3",
