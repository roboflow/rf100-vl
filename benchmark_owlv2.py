...
def run_owlv2_inference(model, processor, image_path, text_queries, threshold=0.1):
    """Run OWLv2 inference on a single image."""
    try:
...
def benchmark_dataset(dataset_path, output_dir, model_name="google/owlv2-large-patch14-ensemble", threshold=0.1, device="cuda", gpu_id=None):
    """Benchmark OWLv2 on a single dataset."""
    ...
    text_queries = [f"a photo of a {name}" for name in category_names]
    ...
    for img_id, img_info in tqdm(images.items(), desc=f"Processing {dataset_name}"):
        ...
        predictions = run_owlv2_inference(model, processor, image_path, text_queries, threshold)
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark OWLv2 on RF100-VL datasets")
    parser.add_argument("--rf100_dir", type=str, default="rf100-vl/rf100-vl",
                        help="Path to RF100-VL datasets directory")
    parser.add_argument("--output_dir", type=str, default="predictions/owlv2",
                        help="Directory to save predictions")
    parser.add_argument("--model_name", type=str, default="google/owlv2-large-patch14-ensemble",
                        help="OWLv2 model name")
