...
def run_gdino_inference(model, processor, image_path, text_queries, threshold=0.1):
...
def benchmark_dataset(dataset_path, output_dir, model, processor, threshold=0.1):
    ...
    for img_id, img_info in tqdm(images.items(), desc=f"Processing {dataset_name}"):
        ...
        predictions = run_gdino_inference(model, processor, image_path, text_queries, threshold)
...
def main():
    ...
    parser.add_argument("--model_name", type=str, default="IDEA-Research/grounding-dino-tiny",
                        help="Grounding DINO model name (tiny/base/large)")
    ...
    model = AutoModelForZeroShotObjectDetection.from_pretrained(args.model_name)
