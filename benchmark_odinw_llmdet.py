...
def find_dataset_root(dataset_path):
    """
    Find the actual dataset root directory within ODinW datasets.
    ODinW datasets may have a nested structure like:
    dataset_name/dataset_name.version/train|test|valid

    Special cases:
    - PascalVOC: uses 'valid' instead of 'test'
    - pistols: uses 'export' directory
    """
    dataset_name = os.path.basename(dataset_path)

    # Special case: pistols uses 'export' directory
    if dataset_name == "pistols":
        export_dir = os.path.join(dataset_path, "export")
        if os.path.exists(export_dir):
            return export_dir

    # Check if dataset_path directly contains train/test/valid
    if os.path.exists(os.path.join(dataset_path, "test")):
        return dataset_path

    # Special case: PascalVOC uses 'valid' instead of 'test'
    if dataset_name == "PascalVOC" and os.path.exists(os.path.join(dataset_path, "valid")):
        return dataset_path
    ...
    for subdir in subdirs:
        ...
        if os.path.exists(os.path.join(subdir_path, "test")):
            return subdir_path

    return None


def benchmark_dataset(dataset_path, output_dir, model_name="iSEE-Laboratory/llmdet_large",
                      threshold=0.1, device="cuda", gpu_id=None, split="test"):
    """Benchmark LLMDet on a single ODinW dataset."""
    dataset_name = os.path.basename(dataset_path)

    # Find the actual dataset root (handle nested structure)
    dataset_root = find_dataset_root(dataset_path)

    if dataset_root is None:
        print(f"Skipping {dataset_name}: Could not find test/train/valid directories")
        return

    # Special case: PascalVOC uses 'valid' instead of 'test'
    if dataset_name == "PascalVOC" and split == "test":
        split = "valid"

    # Special case: pistols has no split (all in export directory)
    if dataset_name == "pistols":
        test_dir = dataset_root
        annotation_path = os.path.join(test_dir, "_annotations.coco.json")
    else:
        test_dir = os.path.join(dataset_root, split)
        annotation_path = os.path.join(test_dir, "_annotations.coco.json")
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark LLMDet on ODinW datasets")
    parser.add_argument("--odinw_dir", type=str, default="/root/odinw",
                        help="Path to ODinW datasets directory")
    parser.add_argument("--output_dir", type=str, default="predictions/odinw_llmdet",
    ...
    if args.datasets:
        ...
        datasets = [d for d in os.listdir(args.odinw_dir)
                   if os.path.isdir(os.path.join(args.odinw_dir, d))]

    print(f"Found {len(datasets)} datasets to process")

    # Process each dataset
    for dataset_name in datasets:
        dataset_path = os.path.join(args.odinw_dir, dataset_name)
